...
public class DrawSWT {
	...
	static private boolean docked, rubber_band_active;

	static private double[] tracker_angle;

	static private Color[] tracker_color;

	static private int tracker_line_width;
	...
	static public boolean isMarkerEnabled() {
		return tracker_gc != null || tracker_canvas != null;
	}
	...
	static public boolean initMarkerPos(int[] center, int[] pos) {
		if (!isMarkerEnabled() || pos == null || (pos.length % 2) == 1) {
			tracker_pos = null;
			return false;
		}
	...
	static public void preparePaint() {
		if (!useNonXorDrawing())
			drawCancel();
	}

	static public void paintMarker(GC gc) {
		if (!useNonXorDrawing() || tracker_pos == null || tracker_x < 0)
			return;
		drawMarkerLines(gc, tracker_x, tracker_y);
	}

	static private void setMarkerPos(int x, int y) {
	...
	static private void drawMarkerLines(int x, int y) {
		drawMarkerLines(tracker_gc, x, y);
	}
	...
	static public boolean isRubberBandEnabled() {
		return rubber_band_gc != null || rubber_band_active;
	}
	...
	static public Rectangle endRubberBandRect() {
		...
		if (useNonXorDrawing()) {
			...
			return (dock_bbox != null) ? dock_bbox : (new Rectangle(
					rubber_band_x, rubber_band_y, rubber_band_width,
					rubber_band_height));
		}
		if (dock_bbox != null && dock_bbox.contains(mouse_x, mouse_y)) {
			rubber_band_gc.drawRectangle(dock_bbox.x, dock_bbox.y,
	...
	static private void redrawMarkerCanvas() {
		if (tracker_canvas != null && !tracker_canvas.isDisposed())
			tracker_canvas.redraw();
	}

	static private boolean useNonXorDrawing() {
