...
class ChartTab {
    ...
    public Composite createTabFolderPage(CTabFolder tab_folder)
    {
        ...
        date_composite.pack();
        Point date_size = date_composite.getSize();
        int birth_width = Math.max(birth.computeSize(SWT.DEFAULT,
                SWT.DEFAULT, true).x, getChildrenWidth(birth));
        int update_width = update.computeSize(SWT.DEFAULT, SWT.DEFAULT,
                true).x;
        int date_width = Math.max(date_size.x, birth_width + update_width);
        date_size.x = date_width;
        date_composite.setSize(date_size);
        GridData date_grid_data = new GridData();
        date_grid_data.widthHint = date_width;
        date_composite.setLayoutData(date_grid_data);
        location.limitWidth(birth_width);
        GridData location_grid_data = new GridData();
        location_grid_data.widthHint = birth_width;
        location.setLayoutData(location_grid_data);
    ...
    private int getChildrenWidth(Composite composite)
    {
        ...
        for (int i = 0; i < children.length; i++)
            width += children[i].computeSize(SWT.DEFAULT, SWT.DEFAULT, true).x;
        return width;
    }
    ...
    private void showDiagram(PaintEvent event, Point size)
    {
        ...
        if (!entry.sameSize(entry.getImage(), size)) {
            ...
            Image old_image = entry.getImage();
            if (old_image != null) {
                old_image.dispose();
                entry.setImage(null);
            }
