...
public class Resource {
    ...
    static private void registerMacLogicalFontAliases()
    {
        String os_name = System.getProperty("os.name", "");
        if (os_name.toLowerCase(Locale.ENGLISH).indexOf("mac") < 0)
            return;
        try {
            Class factory = Class.forName("sun.font.FontManagerFactory");
            Object manager = factory.getMethod("getInstance", new Class[0])
                    .invoke(null, new Object[0]);
            Class cfont_manager = Class.forName("sun.font.CFontManager");
            Method register = cfont_manager.getDeclaredMethod(
                    "registerFont", new Class[] { String.class, String.class });
            register.setAccessible(true);
            register.invoke(manager, new Object[] { "Times New Roman", "Times" });
            register.invoke(manager, new Object[] { "Times New Roman Bold",
                    "Times" });
            register.invoke(manager, new Object[] { "Times New Roman Italic",
                    "Times" });
            register.invoke(manager, new Object[] {
                    "Times New Roman Bold Italic", "Times" });
            register.invoke(manager, new Object[] { "Times New Roman",
                    "Lucida Bright" });
        } catch (Throwable e) {
            // Missing aliases only affect Java 8's startup warning.
        }
    }
    ...
    static private String getFontNameIfAvailable(String name)
    {
        for (int i = 0; i < font_array.length; i++) {
            ...
            if (name.equalsIgnoreCase(font_array[i].getFamily())
                    || name.equalsIgnoreCase(font_array[i]
                            .getFamily(Locale.ENGLISH))
                    || name.equalsIgnoreCase(font_array[i]
                            .getFamily(Locale.TRADITIONAL_CHINESE))
                    || name.equalsIgnoreCase(font_array[i]
                            .getFamily(Locale.SIMPLIFIED_CHINESE))) {
                return font_array[i].getFamily();
            }
        }
        return null;
    }
    ...
    static public String[] getPossibleFontName(String name)
    {
        ...
        for (int i = 0; i < font_array.length; i++) {
            if (name.equalsIgnoreCase(font_array[i].getFontName())) {
                ...
                array[3] = font_array[i].getFamily();
                array[4] = font_array[i].getFamily(Locale.ENGLISH);
                array[5] = font_array[i]
                        .getFamily(Locale.TRADITIONAL_CHINESE);
                array[6] = font_array[i].getFamily(Locale.SIMPLIFIED_CHINESE);
                return array;
            }
            if (name.equalsIgnoreCase(font_array[i].getFamily())) {
                String[] array = new String[8];
                array[0] = font_array[i].getFamily();
                array[1] = font_array[i].getFamily(Locale.ENGLISH);
                array[2] = font_array[i]
                        .getFamily(Locale.TRADITIONAL_CHINESE);
                array[3] = font_array[i].getFamily(Locale.SIMPLIFIED_CHINESE);
                array[4] = font_array[i].getFontName();
                array[5] = font_array[i].getFontName(Locale.ENGLISH);
                array[6] = font_array[i]
                        .getFontName(Locale.TRADITIONAL_CHINESE);
                array[7] = font_array[i].getFontName(Locale.SIMPLIFIED_CHINESE);
                return array;
            }
        }
        return null;
